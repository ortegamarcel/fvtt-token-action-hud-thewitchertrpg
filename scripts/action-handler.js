import { ActionHandler, DELIMITER, Logger, Utils } from "../../token-action-hud-core/scripts/token-action-hud-core.min.js";
import { ACTIONS, WEAPONS_ID } from "./constants.js";

export class MyActionHandler extends ActionHandler {
    ...
    async buildSystemActions(subcategoryIds) {
        ...
        this._getCombat(actor, token.id, { id: WEAPONS_ID, type: 'system' });
    ...
    _getCombat(actor, tokenId, parent) {
        ...
        const actions = actor.items
            ...
            .map(item => ({
                id: item.id,
                name: item.name,
                encodedValue: [ACTIONS.ATTACK, actor.id, tokenId, item.id].join(DELIMITER),
